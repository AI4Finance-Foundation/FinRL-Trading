...
def resample_returns(ret_series: pd.Series, freq: str = "M", how: str = "compound") -> pd.Series:
    ...
    alias = {"M": "ME", "Q": "QE", "Y": "YE"}
    rs_freq = alias.get(freq, freq)
    if how == "compound":
...
def main():
    ...
    # ------------------- Load weights ------------------
    # removed invalid argparse attribute usage; using args.weights_mean below  # robust access
    # argparse turns '-' into '_' in attribute names
