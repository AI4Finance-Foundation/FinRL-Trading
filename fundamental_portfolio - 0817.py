...
def perform_portfolio_optimization(all_stocks_info, all_return_table, trade_date, output_dir):
    ...
    for i, current_trade_date in enumerate(trade_date):
        ...
        # --- 预处理：丢弃缺失值过多的股票列并填充剩余缺失值 ---
        threshold = int(0.8 * len(p1_return_table_pivot))  # 至少 80% 非空
        p1_return_table_pivot = p1_return_table_pivot.dropna(axis=1, thresh=threshold)
        p1_return_table_pivot = (
            p1_return_table_pivot
            .fillna(method="ffill")
            .fillna(method="bfill")
            .fillna(0)
        ...
        selected_stocks = list(
            set(p1_predicted_return.columns).intersection(p1_return_table_pivot.columns)
        )
        if len(selected_stocks) < 3:
            print(f"      ⚠ Too few valid stocks after preprocessing, skip.")
            continue

        p1_predicted_return = p1_predicted_return.loc[:, selected_stocks]
        p1_return_table_pivot = p1_return_table_pivot.loc[:, selected_stocks]

        # 使用 Ledoit-Wolf 收缩的协方差矩阵，提升数值稳定性
        try:
            S = risk_models.CovarianceShrinkage(p1_return_table_pivot).ledoit_wolf()
        except ValueError as e:
            print(f"      ⚠ CovarianceShrinkage failed: {e}")
            # 退化到简单样本协方差，并将 NaN 填 0
            S = risk_models.sample_cov(p1_return_table_pivot).fillna(0)

        # 预测收益向量取第一行（Series, shape=(n,)）
        mu = p1_predicted_return.iloc[0].loc[selected_stocks]

        # ---- 去除无预测收益或协方差缺失的股票 ----
        mu = mu.dropna()
        if len(mu) < 2:
            print(f"      ⚠ Too few stocks after dropping NaN predicted returns, skip.")
            continue

        selected_stocks = list(mu.index)
        # 对协方差矩阵进行同样子集筛选
        S = S.loc[selected_stocks, selected_stocks]

        # ---- Debug 信息 ----
        print(f"      Candidate stocks: {len(selected_stocks)}")
        print(f"      mu NaN count: {mu.isna().sum()}  | Cov matrix NaN count: {S.isna().sum().sum()}")

        # 若协方差仍有 NaN，用 0 填充（更加保守）
        if S.isna().values.any():
            S = S.fillna(0)
            print("      Filled NaNs in covariance matrix with 0.")

        # ---- Mean-Variance (Max Sharpe) 优化 —— 保证凸性 ----
        try:
            ef_mean = EfficientFrontier(mu, S, weight_bounds=(0, 0.05))
            ef_mean.max_sharpe()
            cleaned_weights_mean = ef_mean.clean_weights()
        except ValueError as e:
            print(f"      ⚠ Max Sharpe failed ({e}); fallback to equal weight.")
            cleaned_weights_mean = {t: 1/len(selected_stocks) for t in selected_stocks}
        ...
        # updated Minimum variance optimization  ←   correct way: minimum volatility
        ef_min = EfficientFrontier(None, S, weight_bounds=(0, 0.05)) 
        ef_min.min_volatility()
        cleaned_weights_min = ef_min.clean_weights()


        # Assign weights to stocks
        # 根据更新后的 selected_stocks 重建 idx
        idx = np.isin(p1_alldata.gvkey, selected_stocks)

        p1_alldata["mean_weight"] = 0
        p1_alldata.loc[idx, 'mean_weight'] = [cleaned_weights_mean.get(t, 0) for t in selected_stocks]

        p1_alldata["min_weight"] = 0
        p1_alldata.loc[idx, 'min_weight'] = [cleaned_weights_min.get(t, 0) for t in selected_stocks]

        p1_alldata["equal_weight"] = 0
        p1_alldata.loc[idx, 'equal_weight'] = np.ones(len(selected_stocks)) / len(selected_stocks)
