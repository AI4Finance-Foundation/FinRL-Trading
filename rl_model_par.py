...
import sys
START_METHOD = "fork" if sys.platform.startswith("linux") else None  
import os
...
# suggest: if use GPU, use 16 envs, if not use 1 env
N_ENVS = 16 if USE_GPU else 1
...
def prepare_rolling_train(df, date_column, testing_window, max_rolling_window, trade_date):
    ...
    start = trade_date - max_rolling_window
    end   = trade_date - testing_window
    print(f"[train window] {start} -> {end}")
    train = data_split(df_temp, start, end)
...
def prepare_rolling_test(df, date_column, testing_window, max_rolling_window, trade_date):
    ...
    start = trade_date - testing_window
    print(f"[test  window] {start} -> {end}")
    test=data_split(df_temp, start, end)
...
def train_a2c(agent,USE_GPU ):
    ...
    #policy_kwargs = dict(net_arch=[1024, 1024, 512], ortho_init=False)
    if USE_GPU:
        A2C_PARAMS = {"n_steps": 512, "ent_coef": 0.005, "learning_rate": 0.0002,
         "device": DEVICE}
    else:
        A2C_PARAMS = {"n_steps": 5, "ent_coef": 0.005, "learning_rate": 0.0002}
...
def train_ddpg(agent,USE_GPU ):
   # policy_kwargs = dict(net_arch=[1024, 1024, 512], ortho_init=False)
    ...
    if USE_GPU:
        DDPG_PARAMS = {"batch_size": 2048, "buffer_size": 1000000, "learning_rate": 0.001, 
        "device": DEVICE}
    else:
...
def run_models(df,date_column, trade_date, env_kwargs, 
    ...
    elif USE_GPU and N_ENVS > 1:
        def make_env(seed):
            def _init():
                env = StockPortfolioEnv(df=X_train, **env_kwargs)
                if hasattr(env, "seed"):
                    env.seed(seed)
                return env
            return _init
        #env_train = SubprocVecEnv([make_env(i) for i in range(N_ENVS)])
        try:
            env_train = SubprocVecEnv([make_env(i) for i in range(N_ENVS)], start_method=START_METHOD)
        except Exception as e:
            print("SubprocVecEnv 启动失败,回退 DummyVecEnv。原因:", e)
            from stable_baselines3.common.vec_env import DummyVecEnv
            env_train = DummyVecEnv([lambda: StockPortfolioEnv(df=X_train, **env_kwargs)])


    agent = DRLAgent(env = env_train)
    print(f"[env] {type(env_train).__name__} | num_envs={getattr(env_train, 'num_envs', 1)} | device={DEVICE}")
...
#)
   ...
   #     max_return = sac_return
    ...
    try:
        env_train.close()   # SubprocVecEnv/DummyVecEnv 都有 close()
    except Exception:
        pass
    try:
        e_trade_gym.close()
...
def plot_predict_return_distribution(df_predict_best,sector_name,out_path):
    ...
    for i in range(df_predict_best.shape[0]):
        ...
        ax = fig.gca()
        df_predict_best.iloc[i].hist(ax=ax)

        ax.set_xlabel("predicted return", size=15)
        ax.set_ylabel("frequency", size=15)
        ax.set_title(f"{sector_name}: trade date - {df_predict_best.index[i]}", size=15)

        # unique file name : trade_date + pid
        trade_date_str = str(df_predict_best.index[i]).replace(":", "-").replace(" ", "_").replace("/", "-")
        fname = f"{sector_name}_pred_return_{trade_date_str}_{pid}.png"
        final_path = os.path.join(out_path, fname)
